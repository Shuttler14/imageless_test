...
class Config:
    ...
    API_BASE_URL = os.environ.get("API_BASE_URL", "https://api.mynarrative.store")
...
class handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if path == '/api/creator/payout/initiate':
            ...
            # Mock Stripe response (in production, integrate with actual Stripe)
            stripe_response = {
        ...
        if path == '/api/webhook/shopify-order':
            ...
            if supabase and line_items:
                for item in line_items:
                    ...
                    creator_id = item.get('properties', {}).get('_creator_id')
                    if creator_id:
                        try:
                            # Get creator
    ...
    def do_PUT(self):
        ...
        # =====================================================
        # PUT /api/creator/balance/update (Manual balance update)
        # =====================================================
...
if __name__ == "__main__":
    print("=" * 60)
    print("🎨 My Narrative Creator Economy API")
    print("=" * 60)
